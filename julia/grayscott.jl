...
function grey_scott_step!(U::AbstractMatrix{T}, V::AbstractMatrix{T}, 
                         U_new::AbstractMatrix{T}, V_new::AbstractMatrix{T}, 
                         params::SimParams{T}, dt::T) where T<:AbstractFloat
...
function conv_laplacian(U::AbstractMatrix)
    # Define weights
    c = -1.0   # center
    ...
    left  = circshift(U, (0, -1))
    right = circshift(U, (0, 1))

    up_left    = circshift(U, (-1, -1))
    up_right   = circshift(U, (-1, 1))
    down_left  = circshift(U, (1, -1))
    down_right = circshift(U, (1, 1))
...
function laplacian_v1(U::AbstractMatrix{T}) where T<:AbstractFloat
    # Laplacians with periodic boundary conditions (vectorized)
    lap = circshift(U, (1,0)) .+ circshift(U, (-1,0)) .+
            circshift(U, (0,1)) .+ circshift(U, (0,-1)) .- 4 .* U
    lap
end

function grey_scott_step_v!(U::AbstractMatrix{T}, V::AbstractMatrix{T}, 
                         U_new::AbstractMatrix{T}, V_new::AbstractMatrix{T},
                         params::SimParams{T}, dt::T) where T<:AbstractFloat
    Du, Dv, F, k = params.Du, params.Dv, params.F, params.k
    
    lap_U = conv_laplacian(U)
    lap_V = conv_laplacian(V)
...
function simulation(args)
    ...
    while isopen(screen)    
        t = time_ns()
        grey_scott_step_v!(U, V, U_new, V_new, params, dt)
        st = (time_ns() - t) / 1e3
